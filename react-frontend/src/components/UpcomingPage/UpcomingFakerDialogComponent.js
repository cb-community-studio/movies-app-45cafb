...
import upcomingFakerFactory from "./upcomingFakerFactory";
...
const UpcomingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = upcomingFakerFactory(fakerCount);
        let results = [];
        setLoading(true);
        try {
            for (let i = 0; i < fakeData.length; i++) {
                const _data = fakeData[i];
                const res = await client.service("upcoming").create(_data);
                // const res = _data;
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(UpcomingFakerDialogComponent);
