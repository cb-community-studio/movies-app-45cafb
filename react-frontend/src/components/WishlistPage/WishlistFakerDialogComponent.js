...
import wishlistFakerFactory from "./wishlistFakerFactory";
...
const WishlistFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = wishlistFakerFactory(fakerCount);
        let results = [];
        setLoading(true);
        try {
            for (let i = 0; i < fakeData.length; i++) {
                const _data = fakeData[i];
                const res = await client.service("wishlist").create(_data);
                // const res = _data;
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(WishlistFakerDialogComponent);
